...
from(
	select bbsno,wname, title, viewcnt, wdate, grpno, indent, ansnum,rownum r	--r이라는 이름을 뽑아내려고
	from(  --rownum을 쓸려고 중첩쿼리 사용
		SELECT bbsno, wname, title, content, passwd, viewcnt, wdate, grpno, indent, ansnum 
		FROM bbs
		--where title like '%게시판3%'
		ORDER BY grpno DESC, ansnum 
	)  --테스트>where rownum>=1 and rownum<=5  
)where r>=1 and r<=5     
...
select no,name,phone,skill,filename from team  WHERE name LIKE '%길동%'
order by no desc

INSERT INTO bbs(bbsno, wname, title, content, passwd, wdate, grpno, indent, ansnum)  
VALUES((SELECT NVL(MAX(bbsno), 0) + 1 as bbsno FROM bbs), '왕눈이', '제목', '내용', '123', 
...
select bbsno,grpno,indent,ansnum,refno
from bbs 
order by grpno,indent desc,ansnum;
...
VALUES((SELECT NVL(MAX(bbsno), 0) + 1 as bbsno FROM bbs), '왕눈이', '제목', '내용', '123', 
sysdate, ((SELECT NVL(MAX(grpno), 0) + 1 as grpno FROM bbs)), 0, 0,(select max(bbsno) from bbs),'예외잡기.txt',100); 
